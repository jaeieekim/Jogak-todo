...
- 4px 베이스. 스페이싱 토큰 9종: 4 / 8 / 12 / 16 / 20 / 24 / 32 / 48 / 64. 네이밍은 값 그대로 (`spacing/16` = 16px). 토큰 외 임의 값 금지. (`spacing/64`는 온보딩 등 대형 히어로 화면의 상단 여백용으로 추가 — 일반 콘텐츠 블록 리듬 32–48px보다 큰 여백이 필요한 경우 한정)
...
온보딩 화면의 상단(서비스명+설명)과 핵심 포인트 사이에 배치되는 시연 애니메이션. **무한 루프**로 반복 재생한다(최종 상태 유지 → 페이드아웃 → 재시작).

- **시퀀스**: ① 흰 카드 1장이 영역 세로 중앙에서 scale-y 확장으로 등장(ease-out 400ms, 좌 할일 텍스트 + 우 주요 버튼) → ② 600ms 대기 후 버튼 자동 눌림(`brand/pressed` 전환 + scale 0.98, 96ms)과 함께 버튼 우상단에서 조각 파티클 2개(진한 삼각형 `brand/primary`, 옅은 비정형 사각형 `brand/soft`+얇은 `brand/primary` 외곽선)가 퍼지며 페이드아웃 → ③ 버튼 페이드아웃, 카드엔 텍스트만 남음 → ④ 1차 분열: 스텝2(착수) 카드가 원본 바로 위로 먼저 등장(원본은 아래로 밀림) → 2차 분열: 스텝1(진입) 카드가 맨 위로 등장, 아래 두 장이 함께 한 칸 더 밀려 내려감(각 350ms, `translateY`만 사용) → ⑤ 순차 체크 후 최종 유지 → 페이드아웃 → 재생(무한 루프).
- **최종 배치(위→아래)**: 스텝1(진입) / 스텝2(착수) / 원본 — 실제 체크리스트 순서와 동일. **3장 전부 체크된 상태**로 유지.
- 각 카드 오른쪽에 실제 TodoCard 체크 컨트롤과 같은 스타일의 미니어처 체크(원형, 미체크 `border/strong` 링, 체크 시 `brand/primary` 채움 + 체크마크, scale-in 팝 180ms)를 두고, 분열 완료 500ms 후 위→아래 순서로 순차 체크(스텝1→스텝2 300ms, 스텝2→원본은 400ms로 살짝 벌려 마지막 강조)한다.
- **파티클 색 근거**: 옅은 조각은 `orange/100`이 아니라 기존 `brand/soft`를 쓴다 — orange/100은 예비 Primitive로 아직 semantic이 없고, 일회성 파티클을 위해 토큰을 늘리지 않는다. `brand/soft`는 흰 카드 배경과 대비가 약해 얇은 `brand/primary` 외곽선으로 형태를 보강했고, 두 조각은 비정형 사각형/삼각형으로 모양을 다르게 하고 스폰 위치를 분리해 겹치지 않게 한다.
- **스타일**: 데모 카드 화이트 + `radius/16` + 카드 공통 그림자, 데모 버튼은 주요 버튼 공통 스펙 그대로. 신규 토큰 없음.
- **레이아웃 안정**: 시연 영역 높이는 최종 3장 기준으로 고정 — 애니메이션 중 아래 콘텐츠가 밀리지 않는다. 카드 이동은 `translateY`만 사용해 레이아웃 리플로우가 없다.
- **접근성**: `prefers-reduced-motion: reduce`에서는 루프 없이 최종 상태(3장 전부 체크)만 정적으로 표시한다.
- **노출 조건**: 온보딩 화면 자체의 노출은 첫 방문 1회(홈 진입 시 localStorage 게이트) — 상세는 PRD 6번 "온보딩" 참조. 이 애니메이션은 온보딩 화면이 열릴 때마다 재생.
- 타이밍(딜레이·duration)·파티클 위치·체크 간격은 코드에서 const 상수로 관리해 값만 바꿔 튜닝할 수 있게 한다.
