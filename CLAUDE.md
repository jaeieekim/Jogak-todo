...
- 할 일 카드의 헤더 행(할일명+진행 도트+메뉴) 높이 = **`spacing/32`(32px) 토큰** (체크박스가 체크리스트로 이동해 탭 타겟 근거가 없어졌고, 44px는 토큰이 아니라 상하 여백을 시각적으로 부풀리는 원인이 돼서 축소함). 카드 자체 높이는 체크리스트 항목 수·펼침 여부에 따라 가변.
...
| 할 일 카드 | 카드 하나당 외곽 테두리(`border-border`) 1개, `rounded-12`, 패딩 `px-24px py-20px`(좌우·상하 값 다름). 내부(헤더 행·스텝 행 사이)에 구분선 없음 — 여백만으로 구분. 헤더~체크리스트 간격 `pt-20px`, 체크리스트 항목 간 간격 `gap-4px`. 헤더 행은 높이 `spacing/32`(할일명+진행 도트+메뉴, 체크 기능 없음, 제목-도트 간격 `gap-24px`) — 내부 체크리스트 행에는 고정 높이 적용 안 함(가변). 체크리스트 각 항목은 체크를 오른쪽에 배치(시각 24px, 탭 타겟 `spacing/32`, `-mr-4px`로 시각 정렬 보정 — 잔차 없이 정확히 일치). 미완료 카드는 항상 펼쳐짐(접기 없음), 전체 완료(3/3) 카드는 자동 축소(`bg-bg-tint`, 보더·그림자 없음)되며 탭으로 재펼침 |
| 진행 도트 | 원 3개(●●○), 지름 6px, 간격 `gap-4px`, `rounded-full`, 체크된 만큼 `brand-primary` 채움 나머지 `border-strong` 색 |
